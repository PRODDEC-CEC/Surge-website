...
function EventsSection() {
  ...
  useEffect(() => {
    async function fetchEvents() {
      ...
      const q = query(eventsCol, limit(3)); // limit to 3 docs
        ...
        <div className="relative">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 p-6 lg:p-8">
            {events.length === 0 ? (
              ...
              events.map((event) => {
                const passed = isDatePassed(event.date);
                return (
                  <div
                    key={event.id}
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col flex-grow relative">
                      ...
                      {/* Button */}
                      <button
                        className={`w-full py-3 px-4 rounded-full font-bold transition-all duration-300 transform relative overflow-hidden group/btn
                          ${passed
                            ? "bg-red-500 text-gray-200 cursor-not-allowed"
                            : "bg-red-700 hover:bg-red-600 text-white hover:scale-105"}
                        `}
                        onClick={() => {
                          if (!passed) {
